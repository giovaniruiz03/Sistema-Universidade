# Tarefa - Sistema Universidade (FUNCTIONS)
## METODOLOGIA
Tarefa com o objetivo da criação de um sistema de matrícula para matricular e armazenar os dados do aluno para seu curso designado. 

## Crie um banco de dados para armazenar alunos e cursos de uma universidade;
![image](https://github.com/giovaniruiz03/Sistema-Universidade/assets/145368122/7f8ea927-2419-42d5-bc28-ec4df981bc29)

## O aluno possui um e-mail que deve ter seu endereço gerado automaticamente no seguinte formato: nome.sobrenome@dominio.com
![image](https://github.com/giovaniruiz03/Sistema-Universidade/assets/145368122/b242dbaa-7aa5-45d7-851b-51712fb44cf8)

## Crie uma rotina que recebe os dados de um novo curso e o insere no banco de dados;
![image](https://github.com/giovaniruiz03/Sistema-Universidade/assets/145368122/ba78cab5-4f4f-4692-97ad-e6febe7756df)

## Crie uma função que recebe o nome de um curso e sua área, em seguida retorna o id do curso;
![image](https://github.com/giovaniruiz03/Sistema-Universidade/assets/145368122/a2fab920-d1d8-4efc-b52c-20f7d53b497b)

## Crie uma procedure que recebe os dados do aluno e de um curso e faz sua matrícula;
![image](https://github.com/giovaniruiz03/Sistema-Universidade/assets/145368122/876bd791-929e-4c84-ab00-a41c3000f343)

## Crie o modelo lógico do exercício.

## inserção de Dados e testes
![image](https://github.com/giovaniruiz03/Sistema-Universidade/assets/145368122/fb13df14-75f6-49ee-b0a8-3c072b66acc4)
![image](https://github.com/giovaniruiz03/Sistema-Universidade/assets/145368122/4e0889a3-ea7d-406b-b0bd-795494cf13c6)






 ## Aluno: Giovani Boccardo Ruiz - RA: 223102 - giruiz2003@gmail.com
